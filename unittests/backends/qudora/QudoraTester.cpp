...
std::string mockPort = "8100";
std::string backendStringTemplate =
    "qudora;emulate;false;url;http://localhost:{}/jobs";
...
CUDAQ_TEST(QudoraTester, checkSampleSync) {
  auto backendString =
...
CUDAQ_TEST(QudoraTester, checkSampleAsync) {
  auto backendString =
      fmt::format(fmt::runtime(backendStringTemplate), mockPort);
...
CUDAQ_TEST(QudoraTester, checkSampleAsyncLoadFromFile) {
  auto backendString =
      fmt::format(fmt::runtime(backendStringTemplate), mockPort);
...
CUDAQ_TEST(QudoraTester, checkObserveSync) {
  auto backendString =
      fmt::format(fmt::runtime(backendStringTemplate), mockPort);
...
CUDAQ_TEST(QudoraTester, checkObserveAsync) {
  auto backendString =
      fmt::format(fmt::runtime(backendStringTemplate), mockPort);
...
CUDAQ_TEST(QudoraTester, checkObserveAsyncLoadFromFile) {
  auto backendString =
      fmt::format(fmt::runtime(backendStringTemplate), mockPort);
...
int main(int argc, char **argv) {
  setenv("QUDORA_API_KEY", "00000000000000000000000000000000", 0);
